  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      <Link
        ...
        className="group flex w-full items-center justify-between gap-3 rounded-xl border border-brand-100 bg-gradient-to-r from-brand-50 to-brand-50/40 px-5 py-3 transition-colors hover:border-brand-200"
      >
      ...
      {!loading && jobs.length === 0 && (
        <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center text-sm text-slate-500">
          {canCreateJob ? (
            <>
              No jobs yet.{' '}
              <Link
      ...
      <div className="mt-4 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-card">
        {recentApps.length === 0 ? (
          <p className="px-6 py-12 text-center text-sm text-slate-500">No applicants yet.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/50 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">
            ...
            <tbody className="divide-y divide-slate-50">
              {recentApps.slice(0, 8).map((a, i) => (
                <tr key={a.id} className="transition-colors hover:bg-slate-50/60">
                  <td className="num px-5 py-3.5 text-slate-400">
                  ...
                  <td className="px-5 py-3.5 text-slate-600">{a.jobs?.title ?? '—'}</td>
                  <td className="px-5 py-3.5 text-slate-600">{formatDate(a.created_at)}</td>
                  <td className="px-5 py-3.5">
...
function StatCard({
  ...
  const inner = (
    <>
      ...
      <div className="num mt-2 text-[34px] font-semibold leading-none tracking-tight text-slate-900">
        {value}
      </div>
      <div className="mt-4 inline-flex items-center gap-1 text-[12.5px] font-medium text-emerald-600">
  ...
  const className = cn(
    'flex w-full flex-col rounded-2xl border border-slate-100 bg-white p-6 shadow-card text-left',
    href && 'transition-all hover:border-slate-200 hover:shadow-soft'
  );
  return href ? (
...
function MiniJobCard({ job, apps }: { job: Job; apps: AppWithJob[] }) {
  ...
  return (
    <Link
      ...
      className="group relative block rounded-2xl border border-slate-100 bg-white p-5 text-left shadow-card transition-all hover:border-slate-200 hover:shadow-soft"
    >
      {isNew && (
      ...
      <div className="flex items-start justify-between gap-3">
        <h3
          className={cn(
            'min-w-0 text-[15px] font-semibold leading-tight text-slate-900',
            isNew && 'mt-5'
          )}
        >
          {job.title}
